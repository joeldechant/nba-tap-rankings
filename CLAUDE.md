...
- **Career popup updated** (Mar 17, 2026) — TED/TAP view columns changed from "TOP 10 | High" to "AVG | TOP 10". AVG = average TED/TAP across ALL qualifying players that season (meaningful now with complete player pools). TAPD view unchanged: "TOP 10 | High" (no AVG due to incomplete PM coverage). `avg_ted`/`avg_tap` added to `season_stats` in `build_historical_json.py` and current season merge in `site_generator.py`.
- **Historical PM gap** (Mar 17, 2026) — With complete BR data, 7,232 player-seasons still missing PM data (mostly 2000-2016 where only ~40-50 ranked players had PM, now ~440-490 per year). 2017-2024 mostly complete (20-30 missing each). Scrape needed via `scrape_historical_pm_v2.py` pattern (~24 hours at 12s/request).
