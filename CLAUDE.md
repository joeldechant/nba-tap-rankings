...
- **DIFF tab IMPLEMENTED** (Mar 19, 2026) — "DIFF" nav link (after MG) shows career avg/total TED/TAP DIFF vs top 100 per season. See `memory/diff_tab_params.md` for complete parameter reference. Columns: Rank | Player | aTED/aTAP | aDIFF | tDIFF. Data from 1960+ (matching GOAT/G2/G3). Inclusion: top 400 by aTED/aTAP, then top 200 by aDIFF. Baseline: top 100 avg TED/TAP per season. Default sort by aTED/aTAP descending; click any orange header (aTED/aTAP, aDIFF, tDIFF) to re-sort. `text-transform: none` on stat headers to preserve mixed case (aTED not ATED). Desktop font: headers 0.95em, data 0.92em; mobile: 0.85em. GOAT/G2/G3/MG/DIFF mutually exclusive. Follows TED/TAP toggle (internal `.view-ted`/`.view-tap` divs). `doToggle()` syncs display state. Data: `diff_data_ted`/`diff_data_tap` in `historical_rankings.json` (built by `build_historical_json.py`); `top50_ted/tap` and `top100_ted/tap` added to `season_stats`. Current season merged at site generation time. Render: `render_diff_html()` in `site_generator.py`. CSS: `.diff-table`, shared selectors with GOAT/G2/G3/MG for borders, hover, player ellipsis. **Status: LIVE.**
