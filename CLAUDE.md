...
- **Player of the Month IMPLEMENTED** (Mar 15, 2026) — Click orange season-to-date header to reveal "PLAYER OF THE MONTH TED/TAP" table (top 100). Click monthly header to swap back. Uses `calculate_weekly_rankings(month_start, last_game_date)` — same function as weekly/daily, just with first-of-month as start date. TAP column uses TAPD (same as daily/weekly). Resets at beginning of each month. Implementation mirrors weekly/daily toggle: `.season-monthly-slot` wrapper div, CSS for orange clickable header, JS click handler, state synced in `doToggle()` across TED/TAP views. Minimal additional computation (~1-2 seconds).
  ...
  - **Status: LIVE.**
- **Historical PM scraping STARTED** (Mar 15, 2026) — `scrape_historical_pm.py` scrapes lightweight PM + MP data from BR box scores for seasons 2000-2024. Stores in `historical_pm` table (7 columns vs 18 in game_box_scores). Restartable: skips already-scraped games, resumes from last scraped date per season. Handles special seasons: lockout 2011-12 (Dec 25 start), COVID 2019-20 (bubble through Oct 2020), short 2020-21 (Dec 22 start). Scrapes newest seasons first by default. Status: scraping in progress (~30K games total across 25 seasons). Run `python scrape_historical_pm.py --status` to check progress.
- **Historical TAPD toggle IMPLEMENTED** (Mar 15, 2026) — Third stat mode for historical full-season rankings. In TAP view, clicking the orange "TAP" column sub-header (`th.stat`) in any historical table toggles to TAPD view and back. TAPD uses season-average game PM (from `historical_pm` table) passed through the calculator as `game_plus_minus` to derive OPD → TAPD. For pre-2000 seasons (no PM data), TAPD falls back to showing TAP values with "TAP" column header.
  - **HTML structure**: TAP view wraps historical content in two sibling divs: `.hist-tap-view` (visible by default) and `.hist-tapd-view` (hidden). Complete duplicate HTML sets — same pattern as TED/TAP toggle. TAPD view has independently sorted/ranked players by TAPD value.
  - **Toggle JS**: IIFE-scoped click handler on `.view-tap` parent, delegates to `th.stat` clicks within `.hist-tap-view` or `.hist-tapd-view`. Toggles display of the two wrapper divs.
  - **Decade nav**: Checks if `.hist-tapd-view` is active and scrolls to `-tapd` suffixed decade IDs accordingly.
  - **doToggle() sync**: Resets `.hist-tapd-view` to hidden when switching TED↔TAP (always starts in TAP view).
  - **GOAT/G2/G3**: Work within both wrapper divs (query from active `.view-tap` parent).
  - **`build_historical_json.py` updated**: `load_historical_pm()` queries avg PM from `historical_pm` table. `calculate_tap_for_players()` accepts `pm_lookup` parameter. TAPD values added to: career_data, season_stats (top10_tapd, ldr_tapd, g2_tapd, g3_tapd), per-year player entries, decade top entries, all-time top entries.
  - **`site_generator.py` updated**: `render_historical_section()` accepts `stat_key='tapd'` with fallback to 'tap' for years without PM data. `render_goat_html()`, `render_g2_html()`, `render_g3_html()`, `render_all_time_html()`, `render_decade_top100_html()` all support TAPD with per-year fallback. CSS adds orange/pointer to `th.stat` in `.hist-tap-view` and `.hist-tapd-view` — no problematic title bar overrides.
  - **TAPD is historical-only** — no third toggle for current daily/weekly/monthly rankings (those already use TAPD via `_remap_tapd()`).
  - **Status: Functional once historical PM scraping completes.** Currently falls back to TAP for all years since no PM data yet.
- **Season-to-date TAPD bug fixes** (Mar 15, 2026) — Fixed 3 bugs in the season-to-date TAP/TAPD toggle:
  - **Title bars disappearing on hover**: CSS had `background: transparent` on hover for TAP view title bars, revealing black page background behind black text. Fixed by removing the rule.
  - **Orange color missing from TAP header text**: CSS rule `.view-tap ... .table-section h2 { color: #000 }` was overriding orange. Fixed by removing.
  - **Monthly toggle link missing in TAP view**: JS handler had `if (slot.closest('.view-tap')) return;` which blocked ALL title bar clicks in TAP view. Fixed by removing the guard and targeting specific div types (.season-table or .monthly-table) instead.
  - **TEAM column shifting between TAP/TAPD modes**: "TAPD" header text wider than "TAP" pushed TEAM column. Fixed with `table-layout: fixed` on `.table-section table`, `width: 62px` on `.stat`, `width: 52px` on `.rank`, `width: 50px` on `.team`. Columns now locked in place across all mode switches (desktop and mobile).
  - **TAPD title bar styling**: TAPD title bar ("SEASON-TO-DATE TAPD TOP 100") is white/black and not clickable. TAPD orange `th.stat` column sub-header is the clickable element to return to TAP. TAP and Monthly title bars remain orange/clickable.
  - **Orange greying out on press**: Added explicit `:hover` and `:active` CSS rules on `th.stat` in season-to-date and historical TAP/TAPD views to keep `color: #ee7623; opacity: 1` — prevents browser default active state from dimming the orange.
